 ...
 * 
 * @see本工具所采用的是HttpComponents-Client-4.2.1
 * @see 
 *      ==========================================================================
 *      =========================
 * @see开发HTTPS应用时，时常会遇到两种情况
 ...
 * @see SSLException: hostname in certificate didn't match:
 ...
 *      ==========================================================================
 *      =========================
 * @see这里使用的是HttpComponents-Client-4.2.1创建的连接,所以就要告诉它使用一个不同的TrustManager
 * @see由于SSL使用的模式是X.509,对于该模式,Java有一个特定的TrustManager,称为X509TrustManager
 * @seeTrustManager是一个用于检查给定的证书是否有效的类,所以我们自己创建一个X509TrustManager实例
 * @see而在X509TrustManager实例中
 *      ,若证书无效,那么TrustManager在它的checkXXX()方法中将抛出CertificateException
 * @see既然我们要接受所有的证书,那么X509TrustManager里面的方法体中不抛出异常就行了
 * @see然后创建一个SSLContext并使用X509TrustManager实例来初始化之
 * @see接着通过SSLContext创建SSLSocketFactory,最后将SSLSocketFactory注册给HttpClient就可以了
 * @see 
 *      ==========================================================================
 *      =========================
 * @version v1.7
...
public class HttpsClientUtils {
	 ...
	 * @see若响应消息头中无Content-Type属性,则会使用HttpClient内部默认的ISO-8859-1作为响应报文的解码字符集
	 * @param reqURL
	 *            请求地址(含参数)
	 * @return 远程主机响应正文
	 */
	 ...
	 * @see若响应消息头中未指定Content-Type属性,则会使用HttpClient内部默认的ISO-8859-1
	 * @param reqURL
	 *            请求地址
	 * @param reqData
	...
	public static String sendPostSSLRequest(String reqURL,Integer timout,String contentType, String encodeCharset,String message) {
		...
		HttpClient httpClient = new DefaultHttpClient();
		httpClient.getParams().setParameter(
				CoreConnectionPNames.CONNECTION_TIMEOUT, timout);
	...
	/**https post请求
	 * @param reqURL 请求地址
	 * @param timout 服务器请求超时时间
	 * @param contentType 请求
	 * @param encodeCharset
	 * @param message
	 * @param protocolVersion
	 * @return
	 */
	public static String sendPostSSLRequest(String reqURL,Integer timout,String contentType, String encodeCharset,String message,String protocolVersion) throws NoSuchAlgorithmException, KeyManagementException, IOException {
		String responseContent = null;
		HttpClient httpClient = new DefaultHttpClient();
		httpClient.getParams().setParameter(
				CoreConnectionPNames.CONNECTION_TIMEOUT, timout);
		httpClient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT,timout*2);
		// 创建TrustManager()
		// 用于解决javax.net.ssl.SSLPeerUnverifiedException: peer not authenticated
		X509TrustManager trustManager = new X509TrustManager() {
			@Override
		...
		try {
			// TLS1.0与SSL3.0基本上没有太大的差别,可粗略理解为TLS是SSL的继承者，但它们使用的是相同的SSLContext
			SSLContext sslContext = SSLContext.getInstance(protocolVersion);
			// 使用TrustManager来初始化该上下文,TrustManager只是被SSL的Socket所使用
			sslContext.init(null, new TrustManager[] { trustManager }, null);
			// 创建SSLSocketFactory
			SSLSocketFactory socketFactory = new SSLSocketFactory(sslContext,
